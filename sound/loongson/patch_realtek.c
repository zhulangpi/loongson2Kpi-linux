			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	if (type == AC_WID_AUD_MIX) {
		...
		for (i = 0; i < imux->num_items; i++) {
			...
			ls2h_hda_codec_amp_stereo(codec, nid, HDA_INPUT,
						 imux->items[i].index,
						 HDA_AMP_MUTE, v);
		}
		*cur_val = idx;
		...
		return ls2h_hda_input_mux_put(codec, imux, ucontrol, nid,
			    ...
			    struct snd_ctl_elem_info *uinfo)
{
	struct hda_codec *codec = snd_kcontrol_chip(kcontrol);
	struct alc_spec *spec = codec->spec;
	return ls2h_hda_ch_mode_info(codec, uinfo, spec->channel_mode,
			   ...
			   struct snd_ctl_elem_value *ucontrol)
{
	struct hda_codec *codec = snd_kcontrol_chip(kcontrol);
	struct alc_spec *spec = codec->spec;
	return ls2h_hda_ch_mode_get(codec, ucontrol, spec->channel_mode,
				   spec->num_channel_mode,
			   ...
			   struct snd_ctl_elem_value *ucontrol)
{
	struct hda_codec *codec = snd_kcontrol_chip(kcontrol);
	struct alc_spec *spec = codec->spec;
	int err = ls2h_hda_ch_mode_put(codec, ucontrol, spec->channel_mode,
				      spec->num_channel_mode,
				      &spec->ext_channel_count);
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	unsigned int pinctl = ls2h_hda_codec_read(codec, nid, 0,
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	long *valp = ucontrol->value.integer.value;
	unsigned int val = ls2h_hda_codec_read(codec, nid, 0,
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	unsigned char mask = (kcontrol->private_value >> 16) & 0xff;
	long val = *ucontrol->value.integer.value;
	unsigned int gpio_data = ls2h_hda_codec_read(codec, nid, 0,
			      ...
			      struct snd_ctl_elem_value *ucontrol)
{
	...
	unsigned char mask = (kcontrol->private_value >> 16) & 0xff;
	long val = *ucontrol->value.integer.value;
	unsigned int ctrl_data = ls2h_hda_codec_read(codec, nid, 0,
	...
	if (val==0)
		...
		ctrl_data |= mask;
	ls2h_hda_codec_write_cache(codec, nid, 0, AC_VERB_SET_DIGI_CONVERT_1,
				  ctrl_data);
			      ...
			      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!val)
		...
		ctrl_data |= mask;
	ls2h_hda_codec_write_cache(codec, nid, 0, AC_VERB_SET_EAPD_BTLENABLE,
				  ctrl_data);
			      ...
			      int auto_pin_type)
{
	...
	ls2h_hda_codec_write(codec, nid, 0, AC_VERB_SET_PIN_WIDGET_CONTROL, val);
}
		...
		hda_nid_t nid, int type)
{
	...
	spec = codec->spec;
	ls2hda_array_init(&spec->jacks, sizeof(*jack), 32);
	jack = ls2hda_array_new(&spec->jacks);
	if (!jack)
		return -ENOMEM;
...
static void alc_automute_speaker(struct hda_codec *codec, int pinctl)
{
	...
	for (i = 0; i < ARRAY_SIZE(spec->autocfg.hp_pins); i++) {
		...
		if (!nid)
			break;
		if (ls2h_hda_jack_detect(codec, nid)) {
			spec->jack_present = 1;
			break;
		}
...
static void alc_dual_mic_adc_auto_switch(struct hda_codec *codec)
{
	...
	present = ls2h_hda_jack_detect(codec, spec->ext_mic.pin);
	if (present)
		spec->cur_adc_idx = 1;
	else
		spec->cur_adc_idx = 0;
...
static void alc_mic_automute(struct hda_codec *codec)
{
	...
	if (type == AC_WID_AUD_MIX) {
		/* Matrix-mixer style (e.g. ALC882) */
		ls2h_hda_codec_amp_stereo(codec, cap_nid, HDA_INPUT,
					 alive->mux_idx,
					 HDA_AMP_MUTE, 0);
		ls2h_hda_codec_amp_stereo(codec, cap_nid, HDA_INPUT,
					 dead->mux_idx,
		...
		/* MUX style (e.g. ALC880) */
		ls2h_hda_codec_write_cache(codec, cap_nid, 0,
...
static void alc_init_auto_hp(struct hda_codec *codec)
{
	...
	for (i = 0; i < cfg->hp_outs; i++) {
		snd_printdd("realtek: Enable HP auto-muting on NID 0x%x\n",
			    cfg->hp_pins[i]);
		ls2h_hda_codec_write_cache(codec, cfg->hp_pins[i], 0,
...
static void alc_init_auto_mic(struct hda_codec *codec)
{
	...
	spec->auto_mic = 1;
	ls2h_hda_codec_write_cache(codec, spec->ext_mic.pin, 0,
				  AC_VERB_SET_UNSOLICITED_ENABLE,
...
static int alc_auto_parse_customize_define(struct hda_codec *codec)
{
    ...
    /* TODO: change to platform driver type. by ls2h */
	ass = codec->subsystem_id & 0xffff;
#if 0
	if (ass != codec->bus->dev->subsystem_device && (ass & 1))
		goto do_sku;
#endif
	nid = 0x1d;
...
static void alc_auto_init_digital(struct hda_codec *codec)
{
	...
	for (i = 0; i < spec->autocfg.dig_outs; i++) {
		...
		if (pin) {
			ls2h_hda_codec_write(codec, pin, 0,
					    AC_VERB_SET_PIN_WIDGET_CONTROL,
					    PIN_OUT);
		}
	}
...
static int alc_build_controls(struct hda_codec *codec)
{
	...
	if (!spec->no_analog &&
		...
		err = ls2h_hda_add_vmaster(codec, "Master Playback Switch",
					  NULL, alc_slave_sws);
		if (err < 0)
			return err;
	}
	...
	if (spec->capsrc_nids || spec->adc_nids) {
		...
		for (i = 0; kctl && i < kctl->count; i++) {
			...
			if (!nids)
				nids = spec->adc_nids;
			err = ls2h_hda_add_nid(codec, kctl, i, nids[i]);
			if (err < 0)
				return err;
		}
	}
...
static void alc880_uniwill_mic_automute(struct hda_codec *codec)
{
	...
	present = ls2h_hda_jack_detect(codec, 0x18);
	bits = present ? HDA_AMP_MUTE : 0;
	ls2h_hda_codec_amp_stereo(codec, 0x0b, HDA_INPUT, 1, HDA_AMP_MUTE, bits);
}
...
static void alc880_uniwill_p53_dcvol_automute(struct hda_codec *codec)
{
	...
	present = ls2h_hda_codec_read(codec, 0x21, 0,
				     AC_VERB_GET_VOLUME_KNOB_CONTROL, 0);
	present &= HDA_AMP_VOLMASK;
	ls2h_hda_codec_amp_stereo(codec, 0x0c, HDA_OUTPUT, 0,
				 HDA_AMP_VOLMASK, present);
	ls2h_hda_codec_amp_stereo(codec, 0x0d, HDA_OUTPUT, 0,
				 HDA_AMP_VOLMASK, present);
}
...
static void alc880_medion_rim_automute(struct hda_codec *codec)
{
	...
	if (spec->jack_present)
		ls2h_hda_codec_write(codec, 0x01, 0, AC_VERB_SET_GPIO_DATA, 0);
	else
		ls2h_hda_codec_write(codec, 0x01, 0, AC_VERB_SET_GPIO_DATA, 2);
}
...
static int alc_init(struct hda_codec *codec)
{
	...
	for (i = 0; i < spec->num_init_verbs; i++)
		ls2h_hda_sequence_write(codec, spec->init_verbs[i]);
...
static void alc_free(struct hda_codec *codec)
{
	...
	kfree(spec);
	ls2h_hda_detach_beep_device(codec);
}
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	pin_ctl = ls2h_hda_codec_read(codec, nid, 0,
				     AC_VERB_GET_PIN_WIDGET_CONTROL, 0);
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	old_ctl = ls2h_hda_codec_read(codec, nid, 0,
				     AC_VERB_GET_PIN_WIDGET_CONTROL, 0);
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	sel = ls2h_hda_codec_read(codec, nid, 0, AC_VERB_GET_CONNECT_SEL, 0);
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	sel = ls2h_hda_codec_read(codec, nid, 0, AC_VERB_GET_CONNECT_SEL, 0) & 3;
		       ...
		       int cidx, unsigned long val)
{
	...
	ls2hda_array_init(&spec->kctls, sizeof(*knew), 32);
	knew = ls2hda_array_new(&spec->kctls);
	if (!knew)
		return -ENOMEM;
...
{
	ls2h_hda_codec_write(codec, nid, 0, AC_VERB_SET_PIN_WIDGET_CONTROL,
			    pin_type);
	/* unmute pin */
					      ...
					      int dac_idx)
{
	...
	if (alc880_is_multi_pin(nid)) {
		...
		ls2h_hda_codec_write(codec, alc880_idx_to_selector(idx), 0,
				    AC_VERB_SET_CONNECT_SEL,
...
static void alc880_auto_init_input_src(struct hda_codec *codec)
{
	...
	for (c = 0; c < spec->num_adc_nids; c++) {
		...
		if (!imux->num_items && mux_idx > 0)
			imux = &spec->input_mux[0];
		if (imux)
			ls2h_hda_codec_write(codec, spec->adc_nids[c], 0,
					    AC_VERB_SET_CONNECT_SEL,
					    imux->items[0].index);
	}
}
...
static int alc880_parse_auto_config(struct hda_codec *codec)
{
	...
	err = ls2h_hda_parse_pin_def_config(codec, &spec->autocfg,
					   alc880_ignore);
	if (err < 0)
		return err;
				 ...
				 int num_nids)
{
	...
	for (n = 0; n < num_nids; n++) {
		...
		if (get_wcaps_type(get_wcaps(codec, adc)) != AC_WID_AUD_IN)
			continue;
		cap = adc;
		nconns = ls2h_hda_get_connections(codec, cap, conn,
						 ARRAY_SIZE(conn));
		if (nconns == 1) {
			cap = conn[0];
			nconns = ls2h_hda_get_connections(codec, cap, conn,
...
static int patch_alc880(struct hda_codec *codec)
{
	...
	board_config = ls2h_hda_check_board_config(codec, ALC880_MODEL_LAST,
						  alc880_models,
						  alc880_cfg_tbl);
	if (board_config < 0) {
	...
	err = ls2h_hda_attach_beep_device(codec, 0x1);
	if (err < 0) {
		alc_free(codec);
		return err;
	}
				    ...
				    hda_nid_t mono)
{
	...
	ls2h_hda_codec_write(codec, hp, 0, AC_VERB_SET_PIN_WIDGET_CONTROL,
			    val);
	ls2h_hda_codec_write(codec, line, 0, AC_VERB_SET_PIN_WIDGET_CONTROL,
			    val);
	/* mono (speaker) depending on the HP jack sense */
	val = (val && !spec->jack_present) ? PIN_OUT : 0;
	ls2h_hda_codec_write(codec, mono, 0, AC_VERB_SET_PIN_WIDGET_CONTROL,
			    val);
}
...
static void alc260_hp_automute(struct hda_codec *codec)
{
	...
	spec->jack_present = ls2h_hda_jack_detect(codec, 0x10);
...
static void alc260_hp_3013_automute(struct hda_codec *codec)
{
	...
	spec->jack_present = ls2h_hda_jack_detect(codec, 0x15);
...
{
	unsigned int bits = ls2h_hda_jack_detect(codec, 0x10) ? 0 : PIN_OUT;

	ls2h_hda_codec_write(codec, 0x0f, 0, AC_VERB_SET_PIN_WIDGET_CONTROL,
			    bits);
	ls2h_hda_codec_write(codec, 0x11, 0, AC_VERB_SET_PIN_WIDGET_CONTROL,
			    bits);
	ls2h_hda_codec_write(codec, 0x15, 0, AC_VERB_SET_PIN_WIDGET_CONTROL,
			    bits);
}
...
static void alc260_replacer_672v_automute(struct hda_codec *codec)
{
	...
	present = ls2h_hda_jack_detect(codec, 0x0f);
	if (present) {
		ls2h_hda_codec_write_cache(codec, 0x01, 0,
					  AC_VERB_SET_GPIO_DATA, 1);
		ls2h_hda_codec_write_cache(codec, 0x0f, 0,
					  AC_VERB_SET_PIN_WIDGET_CONTROL,
					  PIN_HP);
	} else {
		ls2h_hda_codec_write_cache(codec, 0x01, 0,
					      ...
					      int sel_idx)
{
	...
	if (nid >= 0x12) {
		int idx = nid - 0x12;
		ls2h_hda_codec_write(codec, idx + 0x0b, 0,
...
static int patch_alc260(struct hda_codec *codec)
{
	...
	board_config = ls2h_hda_check_board_config(codec, ALC260_MODEL_LAST,
						  alc260_models,
						  alc260_cfg_tbl);
	if (board_config < 0) {
...
static void alc882_targa_automute(struct hda_codec *codec)
{
	struct alc_spec *spec = codec->spec;
	alc_automute_amp(codec);
	ls2h_hda_codec_write_cache(codec, 1, 0, AC_VERB_SET_GPIO_DATA,
...
{
	unsigned char bits = ls2h_hda_jack_detect(codec, 0x18) ? HDA_AMP_MUTE : 0;
...
{
	unsigned int present = ls2h_hda_jack_detect(codec, 0x1b);

	ls2h_hda_codec_amp_stereo(codec, 0x14, HDA_OUTPUT, 0,
				 HDA_AMP_MUTE, present ? HDA_AMP_MUTE : 0);
	ls2h_hda_codec_amp_stereo(codec, 0x15, HDA_OUTPUT, 0,
				 HDA_AMP_MUTE, present ? HDA_AMP_MUTE : 0);
}
...
{
	unsigned int present = ls2h_hda_jack_detect(codec, 0x14);
...
static void alc883_clevo_m720_mic_automute(struct hda_codec *codec)
{
	...
	present = ls2h_hda_jack_detect(codec, 0x18);
	ls2h_hda_codec_amp_stereo(codec, 0x0b, HDA_INPUT, 1,
				 HDA_AMP_MUTE, present ? HDA_AMP_MUTE : 0);
}
...
{
	int bits = ls2h_hda_jack_detect(codec, 0x14) ? HDA_AMP_MUTE : 0;
...
{
	int bits = ls2h_hda_jack_detect(codec, 0x1b) ? HDA_AMP_MUTE : 0;
...
static void alc889A_mb31_automute(struct hda_codec *codec)
{
	...
	if (ls2h_hda_codec_read(codec, 0x15, 0, AC_VERB_GET_CONNECT_SEL, 0)
	    == 0x00) {
		present = ls2h_hda_jack_detect(codec, 0x15);
		ls2h_hda_codec_amp_stereo(codec, 0x14,  HDA_OUTPUT, 0,
			HDA_AMP_MUTE, present ? HDA_AMP_MUTE : 0);
		ls2h_hda_codec_amp_stereo(codec, 0x16, HDA_OUTPUT, 0,
			HDA_AMP_MUTE, present ? HDA_AMP_MUTE : 0);
	}
}
					      ...
					      hda_nid_t dac)
{
	...
	if (dac == 0x25)
		...
		return;
	ls2h_hda_codec_write(codec, nid, 0, AC_VERB_SET_CONNECT_SEL, idx);
}
...
static void alc882_auto_init_input_src(struct hda_codec *codec)
{
	...
	for (c = 0; c < spec->num_adc_nids; c++) {
		...
		conns = ls2h_hda_get_connections(codec, nid, conn_list,
...
static int patch_alc882(struct hda_codec *codec)
{
	...
	board_config = ls2h_hda_check_board_config(codec, ALC882_MODEL_LAST,
						  alc882_models,
						  alc882_cfg_tbl);

	if (board_config < 0 || board_config >= ALC882_MODEL_LAST)
		board_config = ls2h_hda_check_board_codec_sid_config(codec,
	...
	if (!spec->adc_nids && spec->input_mux) {
		...
		for (i = 0; i < ARRAY_SIZE(alc882_adc_nids); i++) {
			...
			err = ls2h_hda_get_connections(codec, nid, &cap, 1);
			if (err < 0)
				continue;
			err = ls2h_hda_get_connections(codec, cap, items,
						      ARRAY_SIZE(items));
			if (err < 0)
				continue;
...
static void alc262_hp_master_update(struct hda_codec *codec)
{
	...
	/* HP & line-out */
	ls2h_hda_codec_write_cache(codec, 0x1b, 0,
				  AC_VERB_SET_PIN_WIDGET_CONTROL,
				  val ? PIN_HP : 0);
	ls2h_hda_codec_write_cache(codec, 0x15, 0,
				  AC_VERB_SET_PIN_WIDGET_CONTROL,
				  val ? PIN_HP : 0);
	/* mono (speaker) depending on the HP jack sense */
	val = val && !spec->jack_present;
	ls2h_hda_codec_write_cache(codec, 0x16, 0,
				  AC_VERB_SET_PIN_WIDGET_CONTROL,
				  val ? PIN_OUT : 0);
}
...
static void alc262_hp_bpc_automute(struct hda_codec *codec)
{
	...
	spec->jack_present = ls2h_hda_jack_detect(codec, 0x1b);
...
static void alc262_hippo_master_update(struct hda_codec *codec)
{
	...
	ls2h_hda_codec_amp_stereo(codec, hp_nid, HDA_OUTPUT, 0,
				 HDA_AMP_MUTE, mute);
	/* mute internal speaker per jack sense */
	if (spec->jack_present)
		mute = HDA_AMP_MUTE;
	if (line_nid)
		ls2h_hda_codec_amp_stereo(codec, line_nid, HDA_OUTPUT, 0,
					 HDA_AMP_MUTE, mute);
	if (speaker_nid && speaker_nid != line_nid)
		ls2h_hda_codec_amp_stereo(codec, speaker_nid, HDA_OUTPUT, 0,
					 HDA_AMP_MUTE, mute);
}
...
static void alc262_hippo_automute(struct hda_codec *codec)
{
	...
	spec->jack_present = ls2h_hda_jack_detect(codec, hp_nid);
	alc262_hippo_master_update(codec);
}
...
static void alc262_fujitsu_automute(struct hda_codec *codec, int force)
{
	...
	if (force || !spec->sense_updated) {
		spec->jack_present = ls2h_hda_jack_detect(codec, 0x14) ||
				     ls2h_hda_jack_detect(codec, 0x1b);
	...
	if (spec->jack_present)
		...
		mute = ls2h_hda_codec_amp_read(codec, 0x14, 0, HDA_OUTPUT, 0);
	ls2h_hda_codec_amp_stereo(codec, 0x15, HDA_OUTPUT, 0,
				 HDA_AMP_MUTE, mute);
}
...
static void alc262_lenovo_3000_automute(struct hda_codec *codec, int force)
{
	...
	if (spec->jack_present) {
		...
		mute = ls2h_hda_codec_amp_read(codec, 0x1b, 0, HDA_OUTPUT, 0);
		ls2h_hda_codec_amp_stereo(codec, 0x14, HDA_OUTPUT, 0,
					 HDA_AMP_MUTE, mute);
		ls2h_hda_codec_amp_stereo(codec, 0x16, HDA_OUTPUT, 0,
					 HDA_AMP_MUTE, mute);
				  ...
				  int dir, int idx, long *valp)
{
	...
	for (i = 0; i < 2; i++, valp++)
		change |= ls2h_hda_codec_amp_update(codec, nid, i, dir, idx,
						   HDA_AMP_MUTE,
						   *valp ? 0 : HDA_AMP_MUTE);
	return change;
}
...
static int patch_alc262(struct hda_codec *codec)
{
	...
	board_config = ls2h_hda_check_board_config(codec, ALC262_MODEL_LAST,
						  alc262_models,
						  alc262_cfg_tbl);
...
static void alc268_aspire_one_speaker_automute(struct hda_codec *codec)
{
	...
	present = ls2h_hda_jack_detect(codec, 0x15);
	bits = present ? HDA_AMP_MUTE : 0;
	ls2h_hda_codec_amp_stereo(codec, 0x0f, HDA_INPUT, 0,
				 HDA_AMP_MUTE, bits);
	ls2h_hda_codec_amp_stereo(codec, 0x0f, HDA_INPUT, 1,
				 HDA_AMP_MUTE, bits);
}
...
static void alc268_auto_init_hp_out(struct hda_codec *codec)
{
	...
	if (spec->autocfg.mono_out_pin)
		ls2h_hda_codec_write(codec, spec->autocfg.mono_out_pin, 0,
...
static void alc268_auto_init_mono_speaker_out(struct hda_codec *codec)
{
	...
	if (line_nid == 0x1d || speaker_nid == 0x1d) {
		ls2h_hda_codec_write(codec, speaker_nid, 0,
				    AC_VERB_SET_PIN_WIDGET_CONTROL, PIN_OUT);
		/* mute mixer inputs from 0x1d */
		ls2h_hda_codec_write(codec, 0x0f, 0,
				    AC_VERB_SET_AMP_GAIN_MUTE,
				    AMP_IN_UNMUTE(1));
		ls2h_hda_codec_write(codec, 0x10, 0,
				    AC_VERB_SET_AMP_GAIN_MUTE,
				    AMP_IN_UNMUTE(1));
	} else {
	...
	ls2h_hda_codec_write(codec, 0x02, 0,
			    AC_VERB_SET_AMP_GAIN_MUTE, dac_vol1);
	ls2h_hda_codec_write(codec, 0x03, 0,
...
static int patch_alc268(struct hda_codec *codec)
{
	...
	board_config = ls2h_hda_check_board_config(codec, ALC268_MODEL_LAST,
						  alc268_models,
						  alc268_cfg_tbl);
	...
	if (has_beep) {
		...
		if (!ls2hda_query_amp_caps(codec, 0x1d, HDA_INPUT))
	...
	if (!spec->no_analog && !spec->adc_nids && spec->input_mux) {
		...
		for (i = 0; i < spec->num_adc_nids; i++)
			ls2h_hda_codec_write_cache(codec, alc268_capsrc_nids[i],
...
static void alc269_quanta_fl1_speaker_automute(struct hda_codec *codec)
{
	...
	present = ls2h_hda_jack_detect(codec, 0x15);
	bits = present ? HDA_AMP_MUTE : 0;
	ls2h_hda_codec_amp_stereo(codec, 0x0c, HDA_INPUT, 0,
				 HDA_AMP_MUTE, bits);
	ls2h_hda_codec_amp_stereo(codec, 0x0c, HDA_INPUT, 1,
				 HDA_AMP_MUTE, bits);
...
static void alc269_lifebook_speaker_automute(struct hda_codec *codec)
{
	...
	present |= ls2h_hda_jack_detect(codec, 0x1a);
...
static void alc269_lifebook_mic_autoswitch(struct hda_codec *codec)
{
	...
	present_laptop	= ls2h_hda_jack_detect(codec, 0x18);
	present_dock	= ls2h_hda_jack_detect(codec, 0x1b);

	/* Laptop mic port overrides dock mic port, design decision */
	if (present_dock)
		ls2h_hda_codec_write(codec, 0x23, 0,
...
static int alc269_mic2_mute_check_ps(struct hda_codec *codec, hda_nid_t nid)
{
	...
	if (nid == 0x01 || nid == 0x14) {
		int pinval;
		if (ls2h_hda_codec_amp_read(codec, 0x14, 0, HDA_OUTPUT, 0) &
		    HDA_AMP_MUTE)
			pinval = 0x24;
		else
			pinval = 0x20;
		/* mic2 vref pin is used for mute LED control */
		ls2h_hda_codec_update_cache(codec, 0x19, 0,
					   AC_VERB_SET_PIN_WIDGET_CONTROL,
					   pinval);
	}
...
/* ls2h */
	...
	board_config = ls2h_hda_check_board_config(codec, ALC269_MODEL_LAST,
						  alc269_models,
						  alc269_cfg_tbl);
...
{
	unsigned int present = ls2h_hda_jack_detect(codec, 0x0f);

	ls2h_hda_codec_amp_stereo(codec, 0x16, HDA_INPUT, 0,
				 HDA_AMP_MUTE, present ? HDA_AMP_MUTE : 0);
	ls2h_hda_codec_amp_stereo(codec, 0x1a, HDA_INPUT, 3,
...
static hda_nid_t alc861_look_for_dac(struct hda_codec *codec, hda_nid_t pin)
{
	...
	if (ls2h_hda_get_connections(codec, pin, &mix, 1) != 1)
		return 0;
	num = ls2h_hda_get_connections(codec, mix, srcs, ARRAY_SIZE(srcs));
	if (num < 0)
		return 0;
	for (i = 0; i < num; i++) {
		unsigned int type;
					      ...
					      int pin_type, hda_nid_t dac)
{
	...
	ls2h_hda_codec_write(codec, dac, 0, AC_VERB_SET_AMP_GAIN_MUTE,
			    AMP_OUT_UNMUTE);
	if (ls2h_hda_get_connections(codec, nid, &mix, 1) != 1)
		return;
	num = ls2h_hda_get_connections(codec, mix, srcs, ARRAY_SIZE(srcs));
	if (num < 0)
		return;
...
static int patch_alc861(struct hda_codec *codec)
{
	...
	codec->spec = spec;
        ...
        board_config = ls2h_hda_check_board_config(codec, ALC861_MODEL_LAST,
						  alc861_models,
						  alc861_cfg_tbl);
	...
	err = ls2h_hda_attach_beep_device(codec, 0x23);
	if (err < 0) {
		alc_free(codec);
		return err;
	}
...
static int patch_alc861vd(struct hda_codec *codec)
{
	...
	board_config = ls2h_hda_check_board_config(codec, ALC861VD_MODEL_LAST,
						  alc861vd_models,
						  alc861vd_cfg_tbl);
...
static void alc662_lenovo_101e_ispeaker_automute(struct hda_codec *codec)
{
	...
	present = ls2h_hda_jack_detect(codec, 0x14);
	bits = present ? HDA_AMP_MUTE : 0;
...
static void alc662_lenovo_101e_all_automute(struct hda_codec *codec)
{
	...
	present = ls2h_hda_jack_detect(codec, 0x1b);
	bits = present ? HDA_AMP_MUTE : 0;
...
static void alc663_m51va_speaker_automute(struct hda_codec *codec)
{
	...
	present = ls2h_hda_jack_detect(codec, 0x21);
	bits = present ? HDA_AMP_MUTE : 0;
	ls2h_hda_codec_amp_stereo(codec, 0x0c, HDA_INPUT, 0,
				 HDA_AMP_MUTE, bits);
	ls2h_hda_codec_amp_stereo(codec, 0x0c, HDA_INPUT, 1,
...
static void alc663_21jd_two_speaker_automute(struct hda_codec *codec)
{
	...
	ls2h_hda_codec_amp_stereo(codec, 0x0c, HDA_INPUT, 1,
				 HDA_AMP_MUTE, bits);
	ls2h_hda_codec_amp_stereo(codec, 0x0e, HDA_INPUT, 0,
				 HDA_AMP_MUTE, bits);
	ls2h_hda_codec_amp_stereo(codec, 0x0e, HDA_INPUT, 1,
				 HDA_AMP_MUTE, bits);
}
...
static void alc662_f5z_speaker_automute(struct hda_codec *codec)
{
	...
	ls2h_hda_codec_write(codec, 0x14, 0,
...
static void alc663_two_hp_m1_speaker_automute(struct hda_codec *codec)
{
	...
	present1 = ls2h_hda_jack_detect(codec, 0x21);
	present2 = ls2h_hda_jack_detect(codec, 0x15);

	if (present1 || present2) {
		ls2h_hda_codec_write_cache(codec, 0x14, 0,
...
static void alc663_two_hp_m2_speaker_automute(struct hda_codec *codec)
{
	...
	present1 = ls2h_hda_jack_detect(codec, 0x1b);
	present2 = ls2h_hda_jack_detect(codec, 0x15);
...
static void alc663_two_hp_m7_speaker_automute(struct hda_codec *codec)
{
	...
	present1 = ls2h_hda_codec_read(codec, 0x1b, 0,
			AC_VERB_GET_PIN_SENSE, 0)
			& AC_PINSENSE_PRESENCE;
	present2 = ls2h_hda_codec_read(codec, 0x21, 0,
			AC_VERB_GET_PIN_SENSE, 0)
			& AC_PINSENSE_PRESENCE;
	...
	if (present1 || present2) {
		ls2h_hda_codec_write_cache(codec, 0x14, 0,
			AC_VERB_SET_PIN_WIDGET_CONTROL, 0);
		ls2h_hda_codec_write_cache(codec, 0x17, 0,
			AC_VERB_SET_PIN_WIDGET_CONTROL, 0);
	} else {
		ls2h_hda_codec_write_cache(codec, 0x14, 0,
			AC_VERB_SET_PIN_WIDGET_CONTROL, PIN_OUT);
...
static void alc663_two_hp_m8_speaker_automute(struct hda_codec *codec)
{
	...
	present1 = ls2h_hda_codec_read(codec, 0x21, 0,
			AC_VERB_GET_PIN_SENSE, 0)
			& AC_PINSENSE_PRESENCE;
	present2 = ls2h_hda_codec_read(codec, 0x15, 0,
			AC_VERB_GET_PIN_SENSE, 0)
			& AC_PINSENSE_PRESENCE;
...
//		codec->bus->pci->subsystem_vendor == 0x1025 &&
	...
	board_config = ls2h_hda_check_board_config(codec, ALC662_MODEL_LAST,
						  alc662_models,
				                  alc662_cfg_tbl);
	if (board_config < 0) {
		printk(KERN_INFO "hda_codec: %s: BIOS auto-probing.\n",
...
static void alc680_rec_autoswitch(struct hda_codec *codec)
{
	...
	for (i = 0; i < cfg->num_inputs; i++) {
		nid = cfg->inputs[i].pin;
		if (!(ls2h_hda_query_pin_caps(codec, nid) &
	...
	nid = 0x07;
	if (pin_found)
		ls2h_hda_get_connections(codec, pin_found, &nid, 1);

	if (nid != spec->cur_adc)
		__ls2h_hda_codec_cleanup_stream(codec, spec->cur_adc, 1);
	spec->cur_adc = nid;
	ls2h_hda_codec_setup_stream(codec, nid, spec->cur_adc_stream_tag, 0,
				   spec->cur_adc_format);
}
...
static int alc680_capture_pcm_cleanup(struct hda_pcm_stream *hinfo,
				      ...
				      struct snd_pcm_substream *substream)
{
	ls2h_hda_codec_cleanup_stream(codec, 0x07);
	ls2h_hda_codec_cleanup_stream(codec, 0x08);
	ls2h_hda_codec_cleanup_stream(codec, 0x09);
	return 0;
}
...
static int patch_alc680(struct hda_codec *codec)
{
	...
	board_config = ls2h_hda_check_board_config(codec, ALC680_MODEL_LAST,
						  alc680_models,
						  alc680_cfg_tbl);
 ...
 * patch entries
 */
static struct hda_codec_preset ls2h_hda_preset_realtek[] = {
...
static struct hda_codec_preset_list realtek_list = {
	.preset = ls2h_hda_preset_realtek,
	.owner = THIS_MODULE,
};
